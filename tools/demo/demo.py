...
def depth_vis(depth_np, colored_png_path=None, d_min=0.1, d_max=5.0):
...
def parse_args_to_cfg():
    ...
    # tgt_name = args.video.split('/')[-1].replace('.mp4', '')
    args.tgt_name = args.video.split('/')[-1].replace('.mp4', '')
    ...
    if not static_cam:  # use slam to get cam rotation
        if not Path(paths.slam).exists():
            if not cfg.use_dpvo:
                ...
                print(vo_results.shape, paths.slam)
                ### #[384, 4, 4]
...
def load_data_dict(cfg):
    ...
    R_w2c = camera['cam_c2w'][:, :3, :3]

    K_fullimg = torch.from_numpy(
        np.array([[img_focal, 0, img_center[0]], [0, img_focal, img_center[1]], [0, 0, 1]]).astype(np.float32)
    ).repeat(length, 1, 1)
    R_w2c = torch.from_numpy(R_w2c)
    print(R_w2c.shape, 'RW@shape'*50)
...
def render_incam(cfg):
    ...
    # print(len(pred["smpl_params_incam"])
    ...
    # print(len(pred_c_verts), 'this_is_fuking_bad')
...
def render_global(cfg):
    ...
    intpu_folder_ = str(cfg.paths.incam_video).replace('1_incam.mp4','')
    intpu_folder = os.path.join(intpu_folder_, 'depth_out')
    os.makedirs(intpu_folder, exist_ok=True)
    writer_d = imageio.get_writer(f'{intpu_folder_}/tram_output_depth.mp4', fps=30, mode='I', 
                                format='FFMPEG', macro_block_size=1)

    for i, img_raw in tqdm(enumerate(reader), total=get_video_lwh(video_path)[0], desc=f"Rendering Incam"):
        depth = renderer.render_mesh_depth(verts_incam[i].cuda(), img_raw, [0.8, 0.8, 0.8])
        np.save(f'{intpu_folder}/mesh_depth_{i}.npy', depth)
        depth = depth_vis(depth).astype(np.uint8)
        print(f'saved to {intpu_folder_}')
        writer_d.append_data(depth)
